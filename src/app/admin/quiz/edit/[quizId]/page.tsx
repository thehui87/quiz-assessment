...
interface Question {
  ...
  // Used for tracking changes locally before database IDs are assigned
  tempId: number;
}
...
// (removed unused QuestionUpsert type; we now explicitly build insert/update payloads inline)
...
const QuestionEditor: React.FC<{
  ...
  const { question, qIndex, updateQuestion } = props;
...
export default function AdminQuizEditPage() {
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Split questions into existing (have id) and new (no id).
      const questionsExisting = quiz.questions
        .filter(q => !!q.id)
        .map(q => ({
          id: q.id,
          quiz_id: quiz.id,
      ...
      // Upsert existing (updates) and insert new (inserts). Some DB schemas don't allow
      // inserting with a NULL id via upsert, so we avoid a single upsert for mixed data.
      let upsertedQuestions: Array<{ id: string; text: string }> = [];

      if (questionsExisting.length > 0) {
        const { data, error: upsertQError } = await supabase
          .from("Question")
          .upsert(questionsExisting, { onConflict: "id" })
          .select("id, text");

        if (upsertQError) throw new Error(upsertQError.message || "Failed to upsert questions.");
        upsertedQuestions = upsertedQuestions.concat(data ?? []);
      }

      if (questionsNew.length > 0) {
        const { data: insertedQuestions, error: insertQError } = await supabase
          .from("Question")
          .insert(questionsNew)
          .select("id, text");

        if (insertQError) throw new Error(insertQError.message || "Failed to insert questions.");
        upsertedQuestions = upsertedQuestions.concat(insertedQuestions ?? []);
      }
      ...
      // Build separate lists for existing options (have id) and new options (no id)
      type OptionPayload = {
        id?: string;
        question_id: string;
        text: string;
      ...
      const optionsExisting: OptionPayload[] = [];
      const optionsNew: OptionPayload[] = [];

      quiz.questions.forEach(q => {
        const questionDbId = questionIdMap.get(q.id ?? q.tempId);
        if (!questionDbId) return;

        if (q.type === "single_choice" || q.type === "true_false") {
          q.options.forEach(o => {
            const base = { question_id: questionDbId, text: o.text, is_correct: o.is_correct };
            if (o.id) optionsExisting.push({ id: o.id, ...base });
            else optionsNew.push(base);
          });
          return;
        }
        ...
        if (q.type === "text") {
          const opt = {
            ...
            text: q.options?.[0]?.text || q.text || "",
            is_correct: true,
          };
          if (q.options?.[0]?.id) optionsExisting.push({ id: q.options[0].id, ...opt });
          else optionsNew.push(opt);
        }
      });

      // Upsert existing options (updates) and insert new options (inserts)
      if (optionsExisting.length > 0) {
        const { error: upsertOError } = await supabase
          .from("AnswerOption")
          .upsert(optionsExisting, { onConflict: "id" });
        if (upsertOError)
          throw new Error(upsertOError.message || "Failed to upsert answer options.");
      }

      if (optionsNew.length > 0) {
        const { error: insertOError } = await supabase.from("AnswerOption").insert(optionsNew);
        if (insertOError)
          throw new Error(insertOError.message || "Failed to insert answer options.");
      }
